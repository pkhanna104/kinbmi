function varargout = lfp_mc_mutitask_2(varargin)
...
% Last Modified by GUIDE v2.5 12-Jun-2014 15:41:03
...
addpath('C:\Documents and Settings\Jose M Carmena\My Documents\Preeya\classes')
addpath('C:\Documents and Settings\Jose M Carmena\My Documents\Preeya\utils')
addpath('C:\Documents and Settings\Jose M Carmena\My Documents\Preeya\utils\logdet')

rmpath('C:\Documents and Settings\Jose M Carmena\My Documents\Kelvin\bmiLFP3\clda_helper')
rmpath('C:\Documents and Settings\Jose M Carmena\My Documents\Kelvin\bmiLFP3\utils')
rmpath('C:\Documents and Settings\Jose M Carmena\My Documents\Kelvin\bmiLFP3\classes')
...
% hold(handles.power_hist_disp,'on');
% handles.h_reward        = plot(handles.power_hist_disp,0,0,'k');
% handles.h_init          = plot(handles.power_hist_disp,0,0,'b');
% % handles.h_ctr_holderr   = plot(handles.power_hist_disp,0,0,'r');
% % handles.h_tar_holderr   = plot(handles.power_hist_disp,0,0,'g');
% % handles.h_timeout       = plot(handles.power_hist_disp,0,0,'m');
% legend_strs = {'reward','trials initiated','center hold error',...
%                'target hold error','reach timeout'};
% %legend(handles.power_hist_disp,legend_strs,'location','eastoutside');
% ylim(handles.power_hist_disp,[0 20]);
% set(handles.power_hist_disp,'YTick',0:4:20);
% set(handles.power_hist_disp,'XGrid','on','YGrid','on');

%Channel power plot:
%tmp remove: handles.channel_power_plot
%tmp remove: imagesc(0);
%tmp remove: xlabel('Time')
%tmp remove: ylabel('Frequency')
%tmp remove: set(gca,'Yticklabel',120:0)
%tmp remove: hold(handles.channel_power_plot,'on');
...
data.powererror             = zeros(total_itrs,1);
...
for t = 2:total_itrs
    ...
    if data.n_samp_recv(t) > 0
        ...
        [cur_data,h_plot_data, powerError] = calc_lfp_cursor(p_extractor,data.features(:,t),handles,faux_lfp,data.lfp_target_pos(:,t));
        ...
        handles.task_connect.sendPosVelXY([-1 data.lfp_cursor_kin(1,t) 0 powerError]'); %[X pos, Y pos, empty, powerError flag]  
        data.powererror(t) = powerError;
...
if isfield(decoder.baseline_used,'frac_power')
    handles.fraction = 1;
else
    handles.fraction = 0;
end
...
handles.baseline_convfrac2targ = max(abs(target_positions{2}(2:end,2)));
...
set(handles.assym_beta_box,'Value',handles.assym_beta)
set(handles.assym_beta_totpwr_box,'String',num2str(handles.assym_beta_totpwr))
...
function save_stuff(data, handles)
    ...
    baseline_used=struct();
    if handles.fraction
        baseline_used.frac_power = struct();
        baseline_used.frac_power.frange(1) = str2double(get(handles.low_power_box,'String'));
        baseline_used.frac_power.frange(2) = str2double(get(handles.high_power_box,'String'));
        baseline_used.frac_power.log_flag = handles.log_flag;
        baseline_used.frac_power.frac_range = [handles.frac_range_low handles.frac_range_high];
        baseline_used.frac_power.low_pass_filt = handles.low_pass_filt;
    else
        baseline_used.abs_power = struct();
        baseline_used.abs_power.mean = str2double(get(handles.mean_baseline_used_box,'String'));
        baseline_used.abs_power.targ_step = str2double(get(handles.targ_step_baseline_used_box,'String'));
        baseline_used.abs_power.frange(1) = str2double(get(handles.low_power_box,'String'));
        baseline_used.abs_power.frange(2) = str2double(get(handles.high_power_box,'String'));
        baseline_used.abs_power.log_flag = handles.log_flag;
        baseline_used.abs_power.low_pass_filt = handles.low_pass_filt;
    end
